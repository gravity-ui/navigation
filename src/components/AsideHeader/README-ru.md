...
Управлять состоянием можно через свойства `pinned` и `onChangePinned`, а видимостью кнопки — через `hideCollapseButton`.
...
С помощью настроек `AllPages` пользователи могут скрывать ненужные элементы, задавая им состояние `hidden` (скрыто).
...
Третий аргумент `opts` имеет тип `{ isExpanded: boolean; item: MenuItem; ref: React.RefObject<HTMLElement>; setCollapseBlocker: ((isBlocked: boolean) => void) | undefined }`. Вызывайте `opts.setCollapseBlocker?.(true)` при открытии попапа и `opts.setCollapseBlocker?.(false)` при закрытии, чтобы сайдбар не сворачивался пока попап открыт. В `opts.ref` — ref элемента пункта меню; в `opts.isExpanded` — текущее состояние развёрнутости сайдбара.
...
| collapseButtonWrapper     | Обертка для `CollapseButton`, позволяющая кастомизировать вид дефолтной кнопки.                                 | `(defaultButton: React.ReactNode, data: {isExpanded: boolean; onChangePinned?: (pinned: boolean) => void}) => React.ReactNode` |                           |
| collapseTitle             | Заголовок `CollapseButton` для сворачивания элемента навигации.                                                 |                                                            `string`                                                            | `"Свернуть"` `"Collapse"` |
| pinned                    | Визуальное состояние навигации. При `true` — развёрнуто (закреплено).                                           |                                                           `boolean`                                                            |          `true`           |
...
| onChangePinned            | Обратный вызов, срабатывающий при изменении визуального состояния элемента навигации.                           |                                                  `(pinned: boolean) => void;`                                                  |                           |
...
| onMenuGroupsChanged       | Обратный вызов при изменении состояния сворачивания групп меню.                                                 |                                                `(groups: MenuGroup[]) => void`                                                 |                           |
| renderFooterAfter         | Рендер контента после элементов футера (напр. профиль). Получает setCollapseBlocker.                            |                           `(options?: {setCollapseBlocker?: SetCollapseBlocker}) => React.ReactNode`                           |                           |
| menuGroups                | Группы меню для сворачиваемых секций.                                                                           |                                                         `MenuGroup[]`                                                          |                           |
| defaultMenuGroups         | Начальное состояние групп меню.                                                                                 |                                                         `MenuGroup[]`                                                          |                           |
...
| itemWrapper            | Обёртка элемента меню. Сигнатура: `(p: MakeItemParams, makeItem: (p: MakeItemParams) => React.ReactNode, opts: { isExpanded, item, ref, setCollapseBlocker }) => React.ReactNode`. См. [`types.ts`](../../types.ts). |                         [`ItemWrapper`](https://github.com/gravity-ui/navigation/blob/main/src/components/types.ts#L32-L42)                          |                                 |
| href                   | HTML-атрибут `href`.                                                                                                                                                                                                 |                                                                       `string`                                                                       |                                 |
| onItemClick            | Обратный вызов при клике по элементу. Необязательный 4-й аргумент `options` содержит `setCollapseBlocker(isBlocked)` для блокировки сворачивания сайдбара при открытом попапе.                                       |                         `(item, collapsed, event, options?: { setCollapseBlocker?: (isBlocked: boolean) => void }) => void`                          |                                 |
...
| isExpanded             | При `true` — элемент отображается в развёрнутом виде.                                                                                                                                                                |                                                                      `boolean`                                                                       |                                 |
...
| preloadHeight   | Предзагружает высоту TopAlert на SSR для предотвращения сдвига макета. `true` — использовать оценочную высоту; число задаёт явную высоту в px |                                         `boolean` `number`                                         |        `false`        |
