...
| collapseButtonWrapper     | Обертка для `CollapseButton`, позволяющая кастомизировать вид дефолтной кнопки.                                 | `(defaultButton: React.ReactNode, data: {pinned: boolean; onChangePinned?: (pinned: boolean) => void}) => React.ReactNode` |                           |
| collapseTitle             | Заголовок `CollapseButton` для сворачивания элемента навигации.                                                 |                                                          `string`                                                          | `"Свернуть"` `"Collapse"` |
| pinned                    | Визуальное состояние навигации. При `true` — развёрнуто (закреплено).                                           |                                                         `boolean`                                                          |          `false`          |
...
| pinned                 | При `true` — элемент отображается в развёрнутом (закреплённом) виде.                                                                                                                                                 |                                                                      `boolean`                                                                       |                                 |
